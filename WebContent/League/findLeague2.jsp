...
<%
	...
	//검색관련 처리
	String location = "null".equals(request.getParameter("location")) ? null:request.getParameter("location");
...
<%
		...
		if(startPage!=1){
%>
			<a href="../League/findLeague2.jsp?event=football&category=league&pageNum=<%=1%>&location=<%=location%>&ing=<%=ing%>&search=<%=search%>">≪&nbsp;</a>
			<a href="../League/findLeague2.jsp?event=football&category=league&pageNum=<%=startPage-numOfPage<1?1:startPage-numOfPage%>&location=<%=location%>&ing=<%=ing%>&search=<%=search%>">＜&nbsp;</a>
<%
		}
		// 페이지 번호 처리
		for(int i=startPage; i<=endPage; i++){
%>		
			<a href="../League/findLeague2.jsp?event=football&category=league&pageNum=<%=i%>&location=<%=location%>&ing=<%=ing%>&search=<%=search%>"><%=i%>&nbsp;</a>	
<%		
		}
		//if 문으로 이후 페이지 넘어가게 처리
		if(endPage!=numOfAllPages){
%>
			<a href="../League/findLeague2.jsp?event=football&category=league&pageNum=<%=endPage+1>numOfAllPages?numOfAllPages:endPage+1%>&location=<%=location%>&ing=<%=ing%>&search=<%=search%>">＞&nbsp;</a>
			<a href="../League/findLeague2.jsp?event=football&category=league&pageNum=<%=numOfAllPages%>&location=<%=location%>&ing=<%=ing%>&search=<%=search%>">≫</a>
<%
		}
%>
    ...
    <%}else { // 결과값 있으면 보여주느대~ %>
	  ...
	  <% 
 		...
 		<div align="center">
		<form action="../League/findLeague2.jsp?" method="get" name="searchForm" onsubmit="return check2()">
